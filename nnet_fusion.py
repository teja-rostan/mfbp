...
def fusion_update(w_h, w_h2, w_o, rank):
    ...
    threshold = np.mean([np.percentile(abs(fuser.complete(relations[0])) - abs(w_h.get_value()), 20),
                         np.percentile(abs(fuser.complete(relations[1])) - abs(w_h2.get_value()), 20),
                         np.percentile(abs(fuser.complete(relations[2])) - abs(w_o.get_value()), 20)])
    # degree of pruning (0.8)
    ...
    deg=1-(np.count_nonzero(c1)+np.count_nonzero(c2)+np.count_nonzero(c3))/float(len(c1.flat)+len(c2.flat)+len(c3.flat))

    w_h.set_value(w_h.get_value()*c1)
    w_h2.set_value(w_h2.get_value()*c2)
    w_o.set_value(w_o.get_value()*c3)
    return deg, threshold, c1, c2, c3


def repair(w_h, w_h2, w_o, c1, c2, c3):
    w_h.set_value(w_h.get_value()*c1)
    w_h2.set_value(w_h2.get_value()*c2)
    w_o.set_value(w_o.get_value()*c3)
...
(trX, trY), (teX, teY), num_of_class = load_mnist.load_data()  # load_cifar.load_data()
...
deg, thresh, c1, c2, c3 = fusion_update(w_h, w_h2, w_o, rank)
y_score = predict(teX)
